	...
	public class AccountController : Controller {
		...
		public async Task<ActionResult> Login(LoginViewModel model, string returnUrl) {
			...
			try {
				...
				var isEmailValidated = _authService.IsEmailConfirmed(user.Id);
				if (isEmailValidated) {
		...
		public async Task<ActionResult> ConfirmEmail(string userId, string code) {
			...
			var result = _authService.VerifyEmail(userId, code);
		...
		public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model) {
			...
			if (!_authService.IsEmailConfirmed(user.Id))
				ModelState.AddModelError("", "Bu e-posta adresi için doğrulama kullanıcı tarafından tamamlanmadı");

			if (!ModelState.IsValid) return View(model);

			var code = _authService.GetPasswordResetCode(user.Id);
		...
		public async Task<ActionResult> ResendPassReset(string userId) {
			try {
				var code = _authService.GetPasswordResetCode(userId);
